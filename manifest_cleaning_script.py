...
Optimization rules
------------------
1. Dominator pass (direct-dep only)
   A module X is dropped from depends[] when another module Y in the same
   depends[] list has X in its *direct* graph dependencies (graph[Y]).
   No transitive look-up is performed.

2. Auto-install pass (direct-dep trigger check)
   An auto-install module is dropped only when *every* trigger module
   (i.e. every entry in graph[auto_mod]) already appears in the *direct*
   graph deps of at least one other module in the current depends[] list.
   If even one trigger is not covered that way, the auto-install module is kept.
"""
...
_logger = logging.Logger("manifest_optimizer")
...
def print_kept(mod, reason):
    print(f"  {GREEN}✅ KEEP{RESET}     {BOLD}{mod}{RESET}")
    print(f"              {DIM}↳ {reason}{RESET}")


def print_dropped(mod, reason):
    print(f"  {RED}✂️  DROP{RESET}     {BOLD}{mod}{RESET}")
    print(f"              {DIM}↳ {reason}{RESET}")


def print_promoted(mod, from_mod, reason):
    print(
        f"  {CYAN}⬆️  PROMOTE{RESET}  {BOLD}{mod}{RESET}  {DIM}← from {from_mod}{RESET}"
    )
    print(f"              {DIM}↳ {reason}{RESET}")


def print_diff(original, suggested):
    print_section("DIFF — Current vs Suggested")
    ...
    if removed:
        ...
        for m in removed:
            print(f"    {RED}- {m}{RESET}")
    if added:
        print(f"\n  {GREEN}{BOLD}ADDED ({len(added)}):{RESET}")
        for m in added:
            print(f"    {GREEN}+ {m}{RESET}")
    if unchanged:
        print(f"\n  {DIM}UNCHANGED ({len(unchanged)}):{RESET}")
        for m in unchanged:
            print(f"    {DIM}  {m}{RESET}")
    print(
        f"\n  {BOLD}Summary: {len(original)} → {len(suggested)} modules "
        f"({RED}-{len(removed)}{RESET} / {GREEN}+{len(added)}{RESET}){RESET}\n"
    )
...
def fetch_module_data(session, db_name, uid, port):
    print_section("Building Module Graph from DB")
    rows = _rpc(
        session,
        port,
        db_name,
    ...
    # ── installed modules: full metadata list ─────────────────────────────────
    installed_modules = []
    for r in rows:
            ...
            installed_modules.append(
                {
                    ...
                    "category": cat[1] if isinstance(cat, (list, tuple)) and len(cat) > 1 else "",
                }
            )
    installed_modules.sort(key=lambda x: x["name"])
    print(
        f"  {GREEN}✅ {len(rows)} modules indexed — "
        f"{len(installed_modules)} installed, "
        f"{len(auto_install)} are auto-install.{RESET}\n"
    )
...
# ── Transitive cache (used only for HTML export) ──────────────────────────────

def build_transitive_cache(graph, seeds):
    """Returns a dict: module → set of all transitive deps (for HTML visualiser only)."""
    cache = {}
...
def _direct_deps_of_others(module_list, graph, exclude):
    """
    Return the union of graph[m] (direct deps) for every module m in
    module_list except `exclude`.
    """
    result = set()
...
def dominator_pass(module_list, graph, label):
    """
    Drop a module from depends[] when it appears in the *direct* graph
    dependencies of another module that is already in the depends[] list.

    No transitive look-up is done — only graph[other] is checked.
    """
    print_section(label)
    result = []
    ...
    for mod in module_list:
        ...
        # Which other modules in the list declare `mod` as a direct dep?
        covered_by = [
            other
        ...
        if covered_by:
            print_dropped(mod, f"direct dep of: {sorted(covered_by)}")
        else:
            print_kept(mod, "not a direct dep of any other listed module")
            result.append(mod)
...
def auto_install_unwrap_pass(module_list, graph, auto_install, label, pinned=None):
    """
    Drop an auto-install module when *every* trigger module (entry in
    graph[auto_mod]) is already present in the direct graph deps
    (graph[other]) of at least one other module in the list.

    If even one trigger is missing from those direct deps, the auto-install
    module is kept as an explicit dependency.
    """
    ...
    for mod in module_list:
        if mod not in auto_install or mod in pinned:
            print_kept(mod, "not auto_install — keeping as explicit dep")
            result.append(mod)
            continue
        ...
        if not triggers:
            # Auto-install with no declared triggers always activates — safe to drop.
            print_dropped(mod, "auto_install with no trigger conditions — always active")
            continue

        # Build the union of direct deps declared by every *other* module.
        ...
        if not uncovered:
            print_dropped(
                mod,
                f"auto_install — all triggers {sorted(triggers)} are direct deps "
                f"of other listed modules",
            )
        else:
            print_kept(
                mod,
                f"auto_install — triggers {sorted(uncovered)} not found in direct "
                f"deps of other listed modules",
            )
            result.append(mod)
...
def export_html_tree(
    ...
    def enrich(depends, cache):
        """
        Compute metadata for every module reachable from `depends`.

        dominated_by — populated by the DIRECT-DEP rule used in dominator_pass:
            module X is dominated by Y if X appears in graph[Y] and Y is in `depends`.
        auto_removable — True when every trigger of an auto-install module is a
            direct dep of some other module in `depends` (matches auto_install_unwrap_pass).
        """
        all_trans = set()
        for m in depends:
        ...
        # Reverse map inside reachable set
    ...
    # Annotate each installed module: is it in before/after depends?
...
<title>""" + module_name + r""" — Dependency Analyser</title>
...
renderStats(); renderTree();
    ...
    print(f"\n  {GREEN}✅ HTML saved → {BOLD}{out_path}{RESET}")
    print(f"  {DIM}Open: file://{out_path}{RESET}\n")


# ── Main optimisation loop ─────────────────────────────────────────────────────

def optimize_manifest(module_path, db_name, port):
    print_header("Manifest Dependency Optimizer")
    manifest_path = Path(module_path) / "__manifest__.py"
    if not manifest_path.exists():
        print(f"{RED}❌ __manifest__.py not found at {manifest_path}{RESET}")
        sys.exit(1)
    manifest = literal_eval(manifest_path.read_text(encoding="utf-8"))
    original_depends = manifest.get("depends", [])
    print(f"  {BOLD}Module:{RESET}   {manifest.get('name', '?')}")
    print(f"  {BOLD}Depends:{RESET}  {len(original_depends)} modules\n")

    # ── Step 0: strip blacklist / theme modules ────────────────────────────────
    print_section("Step 0 — Strip Blacklisted / Theme Modules")
    current = []
    for dep in original_depends:
        if dep in UNWANTED_DEPENDS:
            print_dropped(dep, "in UNWANTED_DEPENDS blacklist")
        elif dep.startswith("theme_"):
            print_dropped(dep, "theme module — not relevant in industry manifest")
        else:
            print_kept(dep, "passes pre-filter")
            current.append(dep)
    print(f"\n  After pre-filter: {len(original_depends)} → {len(current)}\n")

    session, uid = authenticate(db_name, port)
    graph, auto_install, installed_modules = fetch_module_data(session, db_name, uid, port)

    # Keep a copy for the HTML "before" view
    before_depends = list(current)
    # Transitive cache is only used for the HTML export and XML scan reachability
    ...
    for iteration in range(1, MAX_ITER + 1):
        ...
        print(f"\n  After dominator: {len(current)} modules\n")

        # Pass B — auto-install trigger check
        current = auto_install_unwrap_pass(
            current, graph, auto_install,
            label=f"Iter {iteration}·B — Auto-Install Trigger Check",
        )
        print(f"\n  After auto-install check: {len(current)} modules\n")

        if set(current) == snapshot:
            print(f"{BOLD}{GREEN}✅ Converged after {iteration} iteration(s).{RESET}\n")
            break
        print(f"  {YELLOW}⟳  Set changed — continuing…{RESET}\n")
    else:
        print(f"{YELLOW}⚠️  Did not converge after {MAX_ITER} iterations.{RESET}\n")

    # ── Build final artefacts ──────────────────────────────────────────────────
    suggested = sorted(set(current))
    after_cache = build_transitive_cache(graph, suggested)
    print_diff(original_depends, suggested)
    ...
    if sorted(original_depends) == suggested:
        print(
            f"{GREEN}{BOLD}✅ Manifest is already optimal — no changes needed.{RESET}\n"
        )
        return
    ...
    answer = (
        input(
            f"{YELLOW}{BOLD}Apply suggested changes to __manifest__.py? (y/N): {RESET}"
        )
        .strip()
        .lower()
    )
    ...
    print(f"\n{GREEN}{BOLD}✅ __manifest__.py updated successfully.{RESET}\n")
