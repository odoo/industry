{
    'name': 'DIY Workshops',
    'version': '1.2',
    'category': 'Services',
    'author': 'Odoo S.A.',
    'depends': [
        'base_industry_data',
        'crm_enterprise',
        'partnership',
        'purchase',
        'sale_subscription',
        'website_event_sale',
        'website_sale_loyalty',
    ],
    'data': [
        'data/res_config_settings.xml',
        'data/ir_attachment_pre.xml',
        'data/product_category.xml',
        'data/product_pricelist.xml',
        'data/res_partner_grade.xml',
        'data/product_product.xml',
        'data/knowledge_cover.xml',
        'data/knowledge_article.xml',
        'data/product_pricelist_item.xml',
        'data/sale_order_template.xml',
        'data/sale_order_template_line.xml',
        'data/mail_message.xml',
        'data/website_menu.xml',
        'data/knowledge_article_favorite.xml',
    ],
    'demo': [
        # Creates the website record and repoints the base.default_website
        # xmlid at it, so it must come before anything resolving that xmlid
        # (demo/product_pricelist.xml).
        'demo/website/website.xml',
        'demo/res_company.xml',
        'demo/product_pricelist.xml',
        'demo/product_product.xml',
        'demo/loyalty_program.xml',
        'demo/crm_lead.xml',
        'demo/mail_activity.xml',
        'demo/res_partner.xml',
        'demo/purchase_order.xml',
        'demo/purchase_order_line.xml',
        'demo/loyalty_reward.xml',
        'demo/sale_order.xml',
        'demo/ir_attachment_post.xml',
        'demo/event_event.xml',
        'demo/event_event_ticket.xml',
        'demo/event_slot.xml',
        'demo/sale_order_line.xml',
        'demo/loyalty_rule.xml',
        'demo/sale_order_post.xml',
        'demo/product_supplierinfo.xml',
        'demo/purchase_order_post.xml',
        'demo/payment_provider_demo.xml',
        'demo/website/views/website_templates.xml',
        'demo/website/views/website_event_templates.xml',
        'demo/website/assets.xml',
        'demo/website/images.xml',
        'demo/website/pages/home.xml',
        'demo/website/pages/the_studio.xml',
        'demo/website/pages/contactus.xml',
        'demo/website/server_actions.xml',
    ],
    'license': 'OEEL-1',
    'cloc_exclude': [
        'data/knowledge_article.xml',
        'demo/website/views/website_templates.xml',
        'demo/website/views/website_event_templates.xml',
        'demo/website/pages/home.xml',
        'demo/website/pages/the_studio.xml',
        'demo/website/pages/contactus.xml',
        'static/src/scss/primary_variables.scss',
    ],
    'application': True,
    'images': [
        'images/main.png',
    ],
    'url': "https://www.odoo.com/trial?industry&selected_app=diy_workshops",
    'website': "https://www.odoo.com/all-industries",
}
