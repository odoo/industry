...
class ComputedFieldsTestCase(TransactionCase):
    ...
    # def test_guest_and_occupancy_computations(self):
    #     partner = self.env['res.partner'].create({'name': 'Booking Partner'})
    #     guest_1 = self.env['res.partner'].create({'name': 'Guest 1'})
    #     guest_2 = self.env['res.partner'].create({'name': 'Guest 2'})
    #     order = self._create_sale_order(partner, in_rental_app=True)

    #     rental_template = self.env['product.template'].create({
    #         'name': 'Rental Item',
    #         'type': 'service',
    #         'rent_ok': True,
    #     })
    ...
    #         'order_id': order.id,
    #         'product_id': rental_template.product_variant_id.id,
    #         'product_uom_qty': 1,
    #         'qty_delivered': 1,
    #         'qty_returned': 0,
    #     })

    #     role = self.env['planning.role'].create({'name': 'Stay Role'})
    #     resource = self.env['resource.resource'].create({
    #         'name': 'Room 101',
    #         'resource_type': 'material',
    #     })
    ...
    #         'resource_id': resource.id,
    #         'sale_line_id': line.id,
    #         'start_datetime': datetime(2026, 2, 7, 15, 0),
    #         'end_datetime': datetime(2026, 2, 8, 11, 0),
    #     })
    ...
    #     self.assertEqual(set(order.x_guests.ids), {guest_1.id, guest_2.id},
    #                      "Order guests should reflect guest lines")
    #     self.assertEqual(resource.x_occupant_ids, guest_1 + guest_2,
    #                      "Resource occupants should include guests assigned to the room")
    #     self.assertIn(resource, guest_1.x_ongoing_bookings,
    #                   "Partner ongoing bookings should include resources where they are occupants")
    #     self.assertEqual(guest_1.x_guests_count, 1,
    #                      "Guest count should match number of guest lines for the partner")

    # def test_ongoing_task_and_stage_clean_computation(self):
    #     resource = self.env['resource.resource'].create({
    #         'name': 'Room 202',
    #         'resource_type': 'material',
    #     })
    #     stage_clean = self.env.ref('booking_engine.project_task_type_17')
    #     stage_other = self.env['project.task.type'].create({'name': 'Other Stage'})
    ...
    #         'name': 'Open Task',
    #         'x_resource_id': resource.id,
    #         'stage_id': stage_clean.id,
    #         'state': '01_in_progress',
    #     })

    #     self.assertEqual(resource.x_ongoing_task_id, open_task,
    #                      "Ongoing task should be the first open task for the resource")
    #     self.assertTrue(resource.x_is_in_stage_clean,
    #                     "Resource should be in clean stage when ongoing task is in clean stage")

    #     open_task.stage_id = stage_other.id
    #     self.assertFalse(resource.x_is_in_stage_clean,
    #                      "Resource should not be in clean stage when ongoing task stage changes")
    ...
    #                      "Cleaning color should be 1 for checkout")

    #     task.x_cleaning = 'stayover'
    #     self.assertEqual(task.x_cleaning_color, 3,
    #                      "Cleaning color should be 3 for non-checkout")
    ...
    #                      "Occupancy color should be 13 for vacant")

    #     resource.x_occupancy = 'occupied'
    #     self.assertEqual(resource.x_occupancy_color, 3,
    #                      "Occupancy color should be 3 for non-vacant")
