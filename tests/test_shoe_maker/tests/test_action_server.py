...
class ShoeMakerAutomationsTestCase(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_1 = cls.env['product.product'].create({
            ...
            'project_template_id': cls.env.ref('planning_field_service_sale_timesheet.fsm_project').id,
        })
    ...
    def test_cancel_task_null_so(self):
        ...
        self.assertEqual(so1.tasks_ids.stage_id.id, self.env.ref('planning_field_service_sale_timesheet.planning_project_stage_4').id,
    ...
    def test_set_state_done_cancel(self):
        ...
        so2.tasks_ids.write({
            'stage_id': self.env.ref('planning_field_service_sale_timesheet.planning_project_stage_3').id,
        })
        ...
        so2.tasks_ids.write({
            'stage_id': self.env.ref('planning_field_service_sale_timesheet.planning_project_stage_4').id,
        })
    ...
    def test_email_sales_order_ready(self):
        ...
        self.so3.tasks_ids[0].state = '1_done'
        ...
        self.so3.tasks_ids[1].state = '1_done'
