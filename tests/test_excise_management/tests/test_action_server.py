...
class ActionServerTestCase(TransactionCase):
    ...
    def test_create_excise_tax_server_action(self):
        ...
        self.assertNotIn(excise_category.x_sales_tax_id, self.fiscal_position.tax_ids.mapped('tax_src_id'),
            "Fiscal positions that are not deposit should not contain the sales taxes of the current category")
        self.assertNotIn(excise_category.x_purchase_tax_id, self.fiscal_position.tax_ids.mapped('tax_src_id'),
            "Fiscal positions that are not deposit should not contain the purchase taxes of the current category")
    ...
    def test_add_excise_taxes_fiscal_position_server_action(self):
        ...
        for tax in excise_taxes:
            self.assertNotIn(tax, self.fiscal_position.tax_ids.mapped('tax_src_id'),
                "All excise taxes should be removed automatically from a fiscal position that is not a fiscal deposit")
        self.fiscal_position.x_is_fiscal_deposit = True
        for tax in excise_taxes:
            self.assertIn(tax, self.fiscal_position.tax_ids.mapped('tax_src_id'),
                "All excise taxes should be added automatically to a fiscal position that becomes a fiscal deposit")
