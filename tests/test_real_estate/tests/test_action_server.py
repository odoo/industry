...
class RealEstateAutomationsTestCase(TransactionCase):
    ...
    def test_contact_side_matches(self):
        ...
        for p in client.x_to_notify_property_ids:
            _logger.info(f"Property to notify: {p.name}")
        ...
        # check send matches server action
        ...
        email = self.env['mail.mail'].search([('recipient_ids.id', '=', client.id)], limit=1)
        self.assertTrue(email, "An email should be sent to the client with matched properties")

    def test_property_side_matches(self):
        property_test = self.env['product.template'].create({
            'name': 'Property',
            'categ_id': self.env.ref('real_estate.product_category_5').id,
            'public_categ_ids': self.env.ref('real_estate.product_public_category_1'),
            'attribute_line_ids': [(0, 0, {
        ...
        property_test.website_ribbon_id = self.env.ref('website_sale.new_ribbon')
        ...
        self.assertEqual(property_test.x_contact_matches_properties[0].id, client_1.id,
                         "Client 1 should be matched with the property")
        self.assertEqual(property_test.x_contact_matches_properties[1].id, client_2.id,
