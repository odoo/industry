...
class ServerActionsTestCase(TransactionCase):
    ...
    def test_make_deposit_storable_delivery_invoice_automation(self):
        self.assertEqual(self.product_template_deposit.invoice_policy, 'order', "The product should have invoice policy to delivery")
        self.assertEqual(self.product_template_deposit.type, 'consu', "The product template should have its type to consu")
        self.assertEqual(self.product_template_deposit.is_storable, False, "The product template should have is_storable to True")
        ...
        self.assertEqual(self.product_template_deposit.invoice_policy, 'delivery', "Changing the product template with x_is_a_deposit should set its invoice policy to delivery")
        self.assertEqual(self.product_template_deposit.type, 'consu', "Changing the product template with x_is_a_deposit should set its type to consu")
        self.assertEqual(self.product_template_deposit.is_storable, True, "Changing the product template with x_is_a_deposit should set its is_storable to True")
    ...
    def test_bom_automation(self):
        self.existing_bom = self.env['mrp.bom'].search([('product_tmpl_id', '=', self.product_template.x_unit_sale_product.id)], limit=1)
        self.assertEqual(self.existing_bom.product_qty, self.product_template.x_quantity_by_deposit_product)
        self.assertEqual(self.existing_bom.type, 'normal')
        self.assertEqual(self.existing_bom.x_parent_product.id, self.product_template.id)
        self.assertEqual(self.existing_bom.x_auto_production, True)
