    ...
    def _check_files_in_path(self, module):
        ...
        for root, dirs, files in os.walk(path):
            ...
            for file_name in files:
                ...
                if 'static/' in file_path and not any(p in file_path for p in ['src/js/', 'src/widget', 'src/scss', 'src/css', 'src/xml']):
    ...
    def _check_context_to_stop_mail_sending(self, root, file_name):
        ...
        for record in root.xpath("//record"):
            ...
            if model_name != 'slide.channel':
                continue
            field_publish_template_id = record.xpath("./field[@name='publish_template_id']")
            warn = False
            if not field_publish_template_id:
                warn = True
            else:
                for field in record.xpath("./field[@name='publish_template_id']"):
                    publish_template_ref = field.get('ref') or field.get('eval')
                    if publish_template_ref != 'False':
                        warn = True
            if warn:
                _logger.warning(
                    "Set publish_template_id to False in file '%s' for model '%s' to prevent mail pollution.",
                    file_name,
                    model_name
                )
        ...
        for function in root.xpath("//function"):
            ...
            try:
                context_dict = literal_eval(raw_context)
