...
class IndustryMailBehaviorTestCase(IndustryCase):
    ...
    def test_generic_mail_message_generated(self):
        mails = self.env['mail.mail']._read_group([('model', '!=', False), ('model', 'not in', EXCEPTION_MODELS), ('state', '!=', 'outgoing')], ['res_id', 'model'], ['__count'])
        ...
        for res_id, res_model, count in mails:
            ...
            if upsell_activity:
                upsell_activity_external_id_map = upsell_activity._get_external_ids()
                upsell_activity_without_external_id_count = sum(1 for a in upsell_activity if not upsell_activity_external_id_map.get(a.id))
                if upsell_activity_without_external_id_count == count:
                    continue
