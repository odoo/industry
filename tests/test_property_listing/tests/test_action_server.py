...
class PropertyListingAutomationsTestCase(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.region_value_1 = cls.env['product.attribute.value'].create({
            'name': 'Value 1',
            'attribute_id': cls.env.ref('property_listing.product_attribute_10').id,
        })
        ...
        cls.condition_value_1 = cls.env['product.attribute.value'].create({
            'name': 'Value 1',
            'attribute_id': cls.env.ref('property_listing.product_attribute_12').id,
        })
        cls.draft_ribbon = cls.env['product.ribbon'].create({
            'name': 'Draft',
            'x_is_draft_ribbon': True,
        })
        cls.pre_published_ribbon = cls.env['product.ribbon'].create({
            'name': 'Pre-published',
        })
        cls.sold_ribbon = cls.env['product.ribbon'].create({
            'name': 'Sold',
            'x_is_closed_ribbon': True,
        })

    def test_create_technical_partner(self):
        product = self.env['product.template'].create({
            'name': 'Test Property',
            'categ_id': self.env.ref('property_listing.product_category_5').id,
    ...
    def test_publish_draft_product(self):
        product = self.env['product.template'].create({
            'name': 'Test Property',
            'website_ribbon_id': self.draft_ribbon.id
        })
        ...
        product.website_ribbon_id = self.pre_published_ribbon.id
        product.is_published = True
    ...
    def test_create_and_archive_appointment_types(self):
        ...
        create_appointment = self.env.ref('property_listing.create_appointment_link_server_action')
        ...
        product.website_ribbon_id = self.sold_ribbon.id
    ...
    def test_send_email_book_visit(self):
        ...
        visit = self.env.ref('property_listing.book_your_visit_server_action')
    ...
    def test_contact_side_matches(self):
        ...
        property_1 = self.env['product.template'].create({
            ...
            'attribute_line_ids': [(0, 0, {
                'attribute_id': self.env.ref('property_listing.product_attribute_10').id,
        ...
        property_3 = self.env['product.template'].create({
            ...
            'public_categ_ids': self.env.ref('property_listing.product_public_category_1'),
            'attribute_line_ids': [(0, 0, {
                'attribute_id': self.env.ref('property_listing.product_attribute_12').id,
        ...
        send_matches_action = self.env.ref('property_listing.send_matches_from_contact_server_action')
    ...
    def test_create_lead_from_match(self):
        ...
        create_lead_action = self.env.ref('property_listing.create_lead_from_match_server_action')
