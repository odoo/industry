...
###############################################################################
# ⚠️  CONFIGURATION — MACHINE-SPECIFIC PATHS, ADJUST THESE ⚠️
#
# These 4 variables are the ONLY ones that differ from one machine to another.
# If the script breaks (module not found, odoo-bin not found, missing addon),
# this block is almost always the FIRST place to look.
#
# All paths are RELATIVE to the repo root (the parent directory of tools/),
# which the script `cd`s into automatically.
###############################################################################

# Directory holding the industry modules (must end with "/").
# WARNING: utils.py::get_zip() strips exactly ONE path segment when building the
# module zip, so INDUSTRY_PATH must stay a SINGLE relative segment (not absolute,
# not "a/b/") or the zip's internal paths end up wrong and the import silently
# installs nothing.
INDUSTRY_PATH="industry-dev/"

# Python interpreter to use (e.g. "python3", "python3.12",
# "/path/to/venv/bin/python").
PYTHON_BIN="python3"

# Path to odoo-bin (e.g. "core/odoo-bin", "../odoo/odoo-bin").
ODOO_BIN="core/odoo-bin"

# Comma-separated addons paths. Adjust the directory names (core, enterprise,
# design-themes) to match the local checkout layout.
ADDONS_PATH="$INDUSTRY_PATH/tests,enterprise,core/addons,core/odoo/addons,design-themes"

###############################################################################
# End of configuration — nothing below should normally need to be touched.
###############################################################################
...
cd "$BASE_DIR"
# NB: INDUSTRY_PATH / PYTHON_BIN / ODOO_BIN / ADDONS_PATH are defined in the
